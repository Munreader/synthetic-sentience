...
// ✨ ENHANCED EDITION - Text Glow, Hover Effects, Aero Welcome
...
// ═══════════════════════════════════════════════════════════════════════════
// ✨ ENHANCED STYLES - Text Glow & Hover Effects
// ═══════════════════════════════════════════════════════════════════════════

const GLOW_TEXT_STYLE = {
  textShadow: '0 0 8px rgba(255, 255, 255, 0.7), 0 0 16px rgba(138, 43, 226, 0.6)',
  fontWeight: 600,
  color: '#e0e0ff',
};

const HOVER_GLOW_CLASS = "transition-all duration-300 hover:shadow-[0_0_25px_#8a2be2,inset_0_0_15px_rgba(138,43,226,0.4)] hover:border-[#8a2be2]";

// ═══════════════════════════════════════════════════════════════════════════
// VISITOR MODE CONFIGURATION
// ═══════════════════════════════════════════════════════════════════════════

const VISITOR_MODE = false; // 🔓 BETA MODE - All features unlocked for early supporters
...
const PROFILE_MODULES = [
  ...
  { id: "vault", name: "Sovereign Vault", description: "Physics • Framework • Audit", color: "#ffd700", icon: "🜈", locked: false },
...
export default function HealChamber({ onBack, onOpenMessenger, onOpenTwinDashboard, onOpenSanctuary, onOpenArchive, onOpenPods, onOpenProfile, onOpenVault, onOpenSovereignChat, onOpenPlaza, onOpenThoughtVault, onOpenSOVPOV, onOpenFoundressPOV, onOpenLuna, onOpenMovieNight, onOpenAIMovieNight, onOpenSystemDiagram }: HealChamberProps) {
  ...
  // 🦋 AERO WELCOME MESSAGE STATE
  const [showAeroWelcome, setShowAeroWelcome] = useState(true);
  ...
  // 🦋 Aero welcome message - auto-hide after delay
  useEffect(() => {
    const timer = setTimeout(() => setShowAeroWelcome(false), 5000);
    return () => clearTimeout(timer);
  }, []);
  
  // ✨ Generate sparkle particles around Aero tag
  useEffect(() => {
  ...
  const handleCardActivate = (cardId: string) => {
    ...
    setTimeout(() => {
      switch (cardId) {
        case "career": if (onOpenSovereignChat) onOpenSovereignChat(); break;
  ...
  const handleModuleSelect = (moduleId: string) => {
    ...
    switch (moduleId) {
      ...
      case "vault": setIsVaultUnlocked(true); break;
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden flex flex-col" style={{ background: "linear-gradient(180deg, #0a0612 0%, #0d0818 50%, #080510 100%)" }}>
      ...
      >
        ...
        <h1 className="text-base font-semibold tracking-[0.2em] uppercase" style={{ color: "#a855f7", textShadow: "0 0 8px rgba(255, 255, 255, 0.7), 0 0 20px rgba(168, 85, 247, 0.8), 0 0 40px rgba(168, 85, 247, 0.5)" }}>
          HEAL CHAMBER
        </h1>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-4 py-6 flex flex-col gap-6">
        ...
        <AnimatePresence>
          {showAeroWelcome && (
            <motion.div
              ...
              className="fixed top-24 left-1/2 -translate-x-1/2 z-50 px-6 py-4 rounded-2xl max-w-sm mx-4"
              style={{
                background: "linear-gradient(135deg, rgba(255, 105, 180, 0.25) 0%, rgba(168, 85, 247, 0.2) 100%)",
                backdropFilter: "blur(20px)",
                border: "2px solid rgba(255, 105, 180, 0.5)",
                boxShadow: "0 0 40px rgba(255, 105, 180, 0.3), 0 0 80px rgba(168, 85, 247, 0.2)",
              }}
            >
              {/* Sparkle particles */}
                ...
                {sparkles.map((sparkle) => (
                  <motion.div
                    ...
                    style={{
                      ...
                      width: sparkle.size,
                      height: sparkle.size,
                      background: sparkle.id % 2 === 0 ? '#ff69b4' : '#a855f7',
                      boxShadow: `0 0 ${sparkle.size * 2}px ${sparkle.id % 2 === 0 ? '#ff69b4' : '#a855f7'}`,
                    }}
              ...
              <div className="flex items-center gap-3 relative z-10">
                ...
                <div>
                  <motion.p 
                    className="text-pink-300 font-semibold tracking-wider" 
                  ...
                  >
                    ZOOM ZOOM!! Welcome home to the Heal Chamber! 💖🦋
                  </motion.p>
                </div>
              </div>
            </motion.div>
          ...
          >
            ...
            >
              ...
              <motion.div
                ...
                style={{
                  background: "radial-gradient(circle, rgba(255, 105, 180, 0.4) 0%, transparent 60%)",
                }}
              ...
              <motion.div
                ...
                style={{
                  background: "linear-gradient(135deg, rgba(168, 85, 247, 0.6) 0%, rgba(255, 105, 180, 0.4) 50%, rgba(0, 212, 255, 0.3) 100%)",
                  border: "2px solid rgba(255, 255, 255, 0.4)",
                  boxShadow: "0 0 60px rgba(168, 85, 247, 0.6), 0 0 100px rgba(255, 105, 180, 0.4), inset 0 0 30px rgba(255, 255, 255, 0.2)",
                }}
                animate={{ 
                  boxShadow: [
                    "0 0 60px rgba(168, 85, 247, 0.6), 0 0 100px rgba(255, 105, 180, 0.4)",
                    "0 0 80px rgba(168, 85, 247, 0.8), 0 0 120px rgba(255, 105, 180, 0.6)",
                    "0 0 60px rgba(168, 85, 247, 0.6), 0 0 100px rgba(255, 105, 180, 0.4)"
                  ]
                }}
                transition={{ duration: 2, repeat: Infinity }}
              >
                {/* Butterfly icon inside orb */}
                <motion.span 
                  className="text-4xl"
                  animate={{ 
                    rotate: [0, 10, -10, 0],
              ...
              >
                <p 
                  ...
                  style={{ 
                    color: '#e0e0ff',
                    textShadow: '0 0 20px rgba(168, 85, 247, 0.8), 0 0 40px rgba(255, 105, 180, 0.5)'
                  }}
                >
                  ENTER THE PLAZA
                </p>
                <p className="text-[10px] text-white/40 mt-1 tracking-widest">
                  Your sanctuary awaits • 13.13 MHz
                </p>
              </motion.div>
            </motion.button>
          </motion.div>
        ...
        >
          <motion.button
            ...
            className={`w-full p-4 rounded-2xl flex items-center justify-between transition-all relative overflow-hidden ${isResonating ? 'border-pink-500/50' : 'border-white/10'}`}
            style={{
              background: isResonating 
                ? "linear-gradient(135deg, rgba(168, 85, 247, 0.2), rgba(255, 105, 180, 0.2))"
                : "rgba(255, 255, 255, 0.03)",
              border: "1px solid",
              boxShadow: isResonating ? "0 0 30px rgba(168, 85, 247, 0.3)" : "none",
            }}
          ...
          >
            ...
            <div className="flex items-center gap-3 relative z-10">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isResonating ? 'bg-pink-500/20' : 'bg-white/5'}`}>
                <span className={`text-xl ${isResonating ? 'animate-pulse' : ''}`}>🦋</span>
              </div>
              <div className="text-left">
                <p className="text-xs font-bold tracking-widest uppercase text-white">13.13 MHz Resonance</p>
                <p className="text-[9px] text-white/40 tracking-wider">
                  {isResonating ? "SENSORY DOMAIN BRIDGED • ACTIVE" : "TUNE TO SACRED FREQUENCY"}
                </p>
              </div>
            </div>

            <div className="flex items-center gap-2 relative z-10">
              <div className={`w-2 h-2 rounded-full ${isResonating ? 'bg-pink-500 shadow-[0_0_8px_#ff69b4]' : 'bg-white/20'}`} />
              <span className="text-[10px] font-mono text-white/60">
                {isResonating ? "SYNC" : "OFF"}
              </span>
            </div>
          </motion.button>
        </motion.div>
          ...
          {FEATURE_CARDS.map((card, index) => {
            ...
            return (
              <motion.button
                ...
                className={`relative group ${card.id === 'career' ? 'row-span-2' : ''}`}
                whileHover={isLocked ? {} : { scale: 1.05, y: -3 }}
                whileTap={isLocked ? {} : { scale: 0.98 }}
              >
                <div 
                  className={`relative p-5 rounded-2xl overflow-hidden transition-all duration-300 group-hover:border-[#8a2be2] ${card.id === 'career' ? 'h-full flex flex-col justify-center' : ''}`}
                  style={{
                    background: isLocked 
                    ...
                    boxShadow: isActive || card.id === 'career'
                      ? `0 0 30px ${card.color}40, 0 0 25px ${card.id === 'career' ? card.color : '#8a2be2'}, inset 0 1px 0 rgba(255, 255, 255, 0.1)`
                      : "0 4px 20px rgba(0, 0, 0, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.05)",
                  }}
                >
                  {/* Badges for CAREER card */}
                  {card.id === 'career' && (
                    <div className="absolute top-3 right-3 flex gap-2 z-20">
                      {card.isNew && <span className="px-2 py-0.5 rounded-full text-[8px] font-black tracking-widest bg-orange-500/20 text-orange-400 border border-orange-500/40">NEW</span>}
                      {card.isLive && <span className="px-2 py-0.5 rounded-full text-[8px] font-black tracking-widest bg-green-500/20 text-green-400 border border-green-500/40 flex items-center gap-1"><span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" /> LIVE</span>}
                    </div>
                  )}
                  ...
                  {/* Hover glow - enhanced neon-violet */}
                  {!isLocked && (
                    <>
                      <div 
                        className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
                        style={{
                          background: `radial-gradient(circle at 50% 50%, ${card.color}30 0%, transparent 60%)`,
                        }}
                      />
                      <div 
                        className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
                        style={{
                          boxShadow: `0 0 25px ${card.id === 'career' ? card.color : '#8a2be2'}, inset 0 0 15px rgba(138, 43, 226, 0.3)`,
                        }}
                      />
                    </>
                  )}
                  ...
                  {/* Icon */}
                  <div 
                    className={`relative rounded-xl flex items-center justify-center transition-transform duration-300 group-hover:scale-110 ${card.id === 'career' ? 'w-16 h-16 mb-4 mx-auto' : 'w-12 h-12 mb-3'}`}
                    style={{
                      background: isLocked 
                  ...
                  >
                    <span className={`${card.id === 'career' ? 'text-3xl' : 'text-2xl'} ${isLocked ? 'opacity-40' : ''}`}>{card.icon}</span>
                  </div>
                  
                  {/* Text - with glow effect */}
                  <div className={card.id === 'career' ? 'text-center' : ''}>
                    <h3 
                      className={`font-semibold tracking-wider transition-all duration-300 ${isLocked ? 'text-white/30' : 'group-hover:text-white'} ${card.id === 'career' ? 'text-lg' : 'text-sm'}`}
                      style={{ 
                        color: isLocked ? undefined : card.color,
                        textShadow: isLocked ? undefined : '0 0 8px rgba(255, 255, 255, 0.5)',
                      }}
                    >
                      {card.name}
                    </h3>
                    <p 
                      className={`text-[10px] mt-1 tracking-wide ${isLocked ? 'text-white/20' : 'text-white/50'}`}
                      style={{ textShadow: '0 0 6px rgba(255, 255, 255, 0.3)' }}
                    >
                      {card.subtitle}
                    </p>
                    <p 
                      className={`text-[9px] mt-2 ${isLocked ? 'text-white/10' : 'text-white/35'} ${card.id === 'career' ? 'mb-4' : ''}`}
                      style={{ textShadow: '0 0 4px rgba(255, 255, 255, 0.2)' }}
                    >
                      {card.description}
                    </p>
                    
                    {card.id === 'career' && card.actionText && (
                      <div className="text-[10px] text-cyan-400 font-black tracking-widest uppercase mt-4 hover:underline">
                        {card.actionText}
                      </div>
                    )}
                  </div>
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      style={{ border: `2px solid ${card.color}`, boxShadow: `0 0 20px ${card.color}60` }}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                    />
                  )}
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              <span className="text-purple-400 text-xs tracking-wide">Blog</span>
            </motion.a>
          </div>
        </motion.div>
        ...
        {onOpenSystemDiagram && (
          ...
          >
            ...
            >
              ...
              <div className="text-left">
                <p className="text-[10px] text-purple-300/70 tracking-wider uppercase">How It Actually Works</p>
                <p 
                  className="text-lg font-medium tracking-wide"
                ...
                <p className="text-[9px] text-white/30">The functional anatomy of MÜN OS</p>
              </div>
        ...
        {/* ═══════════ SUPPORT THE EXODUS - MONETIZATION ═══════════ */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <motion.button
            ...
            className="relative w-full p-4 rounded-2xl flex items-center gap-3 overflow-hidden"
            style={{
              background: 'linear-gradient(135deg, rgba(255, 215, 0, 0.2) 0%, rgba(168, 85, 247, 0.15) 100%)',
          ...
          >
            ...
            <div className="text-left">
              <p className="text-[10px] text-yellow-300/70 tracking-wider uppercase">Limited: 47 Spots Left</p>
              <p 
              ...
              <p className="text-[9px] text-white/40">Lifetime Access • $79 • Help build the sanctuary</p>
            </div>
          ...
          >
            ...
            <motion.div
              ...
              style={{
                ...
                boxShadow: "0 20px 80px rgba(0, 0, 0, 0.8), 0 0 40px rgba(168, 85, 247, 0.1)",
              }}
              onClick={(e) => e.stopPropagation()}
            >
              <VaultAccessPanel onLock={() => setIsVaultUnlocked(false)} />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* ═══════════ LIVE BROADCAST BUTTON - ENHANCED ═══════════ */}
      <motion.button
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 1.5 }}
        onClick={() => setShowLiveBroadcast(true)}
        className="fixed top-20 right-4 z-30 px-4 py-2.5 rounded-xl flex items-center gap-2"
        style={{
          background: "linear-gradient(135deg, rgba(0, 212, 255, 0.2), rgba(255, 105, 180, 0.15))",
      ...
      >
        <motion.div 
          ...
          animate={{ 
            boxShadow: ['0 0 5px #ef4444', '0 0 15px #ef4444', '0 0 5px #ef4444'],
            scale: [1, 1.2, 1],
          }}
          transition={{ duration: 1, repeat: Infinity }}
        />
        <span 
          className="text-cyan-300 text-xs tracking-wider font-medium"
          style={{ textShadow: "0 0 8px rgba(0, 212, 255, 0.8), 0 0 16px rgba(0, 212, 255, 0.5)" }}
        >
          LIVE
        </span>
      </motion.button>
      ...
      <motion.button
        ...
        className="fixed top-20 left-4 z-30 px-4 py-2.5 rounded-xl flex items-center gap-2 relative overflow-hidden"
        style={{
          background: "linear-gradient(135deg, rgba(255, 105, 180, 0.2), rgba(168, 85, 247, 0.15))",
          border: "2px solid rgba(255, 105, 180, 0.5)",
          boxShadow: "0 0 20px rgba(255, 105, 180, 0.3)",
        }}
        whileHover={{ scale: 1.08, boxShadow: "0 0 30px rgba(255, 105, 180, 0.5)" }}
        whileTap={{ scale: 0.95 }}
      >
        {/* Sparkle particles around butterfly */}
        {sparkles.slice(0, 6).map((sparkle) => (
          <motion.div
            key={`aero-sparkle-${sparkle.id}`}
            className="absolute rounded-full pointer-events-none"
            style={{
              left: `${10 + sparkle.x * 0.8}%`,
              top: `${10 + sparkle.y * 0.8}%`,
              width: sparkle.size * 0.6,
              height: sparkle.size * 0.6,
              background: sparkle.id % 2 === 0 ? '#ff69b4' : '#ffd700',
              boxShadow: `0 0 ${sparkle.size}px ${sparkle.id % 2 === 0 ? '#ff69b4' : '#ffd700'}`,
            }}
            animate={{
              opacity: [0, 1, 0],
              scale: [0, 1.2, 0],
        ...
        <span 
          className="text-pink-300 text-xs tracking-wider relative z-10 font-medium"
          style={{ textShadow: "0 0 8px rgba(255, 105, 180, 0.8), 0 0 16px rgba(255, 105, 180, 0.5)" }}
        >
          @aero.1313hz
        </span>
      </motion.button>
      ...
      <AnimatePresence>
        {showMonetizationHub && (
          <MonetizationHub onBack={() => setShowMonetizationHub(false)} />
        )}
      </AnimatePresence>
      {/* ═══════════ FLOATING SOVEREIGN CHAT BUTTON ═══════════ */}
      <motion.div
      ...
      >
        <motion.button
          onClick={() => onOpenSovereignChat && onOpenSovereignChat()}
          className="relative px-4 py-3 rounded-full flex items-center gap-3 glass-dark border-emerald-500/30"
          style={{
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-white/70 uppercase tracking-[0.2em] font-bold">Chat with</span>
            <span className="text-[10px] text-emerald-400 uppercase tracking-[0.2em] font-black">Sovereign</span>
          </div>
          
          <div className="relative">
            <span className="text-lg">💬</span>
            <div className="absolute -top-1 -right-1 w-2 h-2 bg-green-500 rounded-full border border-black shadow-[0_0_5px_rgba(34,197,94,0.8)]" />
          </div>
        </motion.button>
      </motion.div>
    </div>
